...
void baseRateTask(void *arg)
{
  ...
  noErr = (rtmGetErrorStatus(px4demo_attitude_control2019_M) == (NULL));
  int sem_value;
  while (noErr ) {
    ...
    px4demo_attitude_control20190311roll_step();
...
void terminateTask(void *arg)
{
  ...
  /* Terminate model */
  px4demo_attitude_control20190311roll_terminate();
  sem_post(&stopSem);
}
...
void SpawnThreads_Task(void *arg)
{
  px4demo_attitude_control20190311roll_initialize();
  sleep(1);
...
void ShutDown_Procedure()
{
  ...
  rtmSetErrorStatus(px4demo_attitude_control2019_M, "Module finished");
