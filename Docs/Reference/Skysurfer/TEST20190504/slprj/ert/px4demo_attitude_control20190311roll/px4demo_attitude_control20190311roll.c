...
/* Output and update for referenced model: 'px4demo_attitude_control20190311roll' */
void px4demo_attitude_control2019031(const real32_T *rtu_roll, const real32_T
  ...
  *rtu_ch6, const real32_T *rtu_P, const real32_T *rtu_Q, uint16_T rty_PWM[4],
  ...
  DW_px4demo_attitude_control_f_T *localDW)
{
  ...
  *rty_mode = localDW->Delay1_DSTATE;
   ...
   *  Gain: '<S3>/Gain2'
   */
  localB->Sum1 = ((real_T)(1.0F * localB->sf_MATLABFunction3.y) -
                  localB->sf_MATLABFunction4.y) + localB->rty_mode;
   ...
   */
  localB->Sum = 1.10009301262288 * localB->Sum1 + localDW->Integrator_DSTATE;
   ...
   */
  localB->Sum2 = localB->Saturate - rtb_DataTypeConversion;
  ...
  *rty_rollout = (real32_T)localB->Saturate_o;
  ...
  rtb_DataTypeConversion5 = *rtu_pitch;
   ...
   *  Gain: '<S3>/Gain'
   */
  rtb_Sum_gx = 1.0F * localB->sf_MATLABFunction2.y -
    localB->sf_MATLABFunction1.y;
   ...
   *  Sum: '<S3>/Sum'
   */
  localB->Sum_m = (1.0F * localB->sf_MATLABFunction2.y -
                   localB->sf_MATLABFunction1.y) * 1.8545410952272 +
    localDW->Integrator_DSTATE_n;
   ...
   */
  localB->Sum3 = localB->Saturate_d - rtb_DataTypeConversion1_j;
  ...
  *rty_pitchout = (real32_T)localB->Saturate_h;
  if (rtmIsSampleHit(1, 0)) {
     ...
     */
    rtb_DataTypeConversion11 = (real32_T)localDW->UnitDelay4_DSTATE;
  }
  ...
  if (rtmIsSampleHit(1, 0)) {
    /* Gain: '<Root>/Gain5' */
    localB->Gain5 = 1.0F * rtb_DataTypeConversion11;
   ...
   */
  if (*rty_mode) {
    localB->Switch1[0] = 0.0F;
  } else {
    localB->Switch1[0] = *rtu_roll;
  }

  if (*rty_mode) {
    localB->Switch1[1] = 0.0F;
  } else {
    localB->Switch1[1] = *rtu_pitch;
  }

  if (*rty_mode) {
    localB->Switch1[2] = 0.0F;
  } else {
    localB->Switch1[2] = rtb_DataTypeConversion;
  }

  if (*rty_mode) {
    localB->Switch1[3] = 0.0F;
  } else {
    localB->Switch1[3] = rtb_DataTypeConversion1_j;
  }
  ...
  *rty_ARM_Control = localDW->Delay_DSTATE;
  ...
  localDW->Integrator_DSTATE_n += ((localB->Saturate_d - localB->Sum_m) * 1.0 +
    2.0387841436751 * rtb_Sum_gx) * 0.004;
