...
void baseRateTask(void *arg)
{
  ...
  noErr = (rtmGetErrorStatus(px4demo_attitude_system20280_M) == (NULL));
  int sem_value;
  while (noErr ) {
    ...
    px4demo_attitude_system20280822car_step();
...
void terminateTask(void *arg)
{
  ...
  /* Terminate model */
  px4demo_attitude_system20280822car_terminate();
  sem_post(&stopSem);
}
...
void SpawnThreads_Task(void *arg)
{
  px4demo_attitude_system20280822car_initialize();
  sleep(1);
...
void ShutDown_Procedure()
{
  ...
  rtmSetErrorStatus(px4demo_attitude_system20280_M, "Module finished");
